...
var (
	appBackupDir   string
	SystemWidePath = "/Applications/HushBeam-Ollama.app"
)
...
func init() {
	...
	Installer = "HushBeam-Ollama-darwin.zip"
	...
	// Executable = HushBeam-Ollama.app/Contents/MacOS/HushBeam-Ollama

	UpgradeLogFile = filepath.Join(home, "Library", "Application Support", "app.reckr.hushbeam.macos", "logs", "upgrade.log")
	...
	appDataDir := filepath.Join(cacheDir, "hushbeam-ollama")
...
func DoUpgrade(interactive bool) error {
	...
	appBackup := filepath.Join(appBackupDir, "HushBeam-Ollama.app")
	...
	// Bundle contents HushBeam-Ollama.app/Contents/...
	links := []*zip.File{}
	...
	for _, f := range links {
		s := strings.SplitN(f.Name, "/", 2) // Strip off HushBeam-Ollama.app/
		if len(s) < 2 || s[1] == "" {
		...
		// Don't allow links outside of HushBeam-Ollama.app
...
func verifyDownload() error {
	...
	if err := verifyExtractedBundle(filepath.Join(dir, "HushBeam-Ollama.app")); err != nil {
...
func alreadyMoved() string {
	...
	installedAppPaths, err := filepath.Glob(filepath.Join(
		...
		"Contents", "MacOS", "HushBeam-Ollama"))
	if err != nil {
